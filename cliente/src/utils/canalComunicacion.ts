 ...
 */
export const peticionComunicacion = ( data : DataCanalPeticion) => {
    try {
        const canalComunicacion = new BroadcastChannel( data.nombreCanal );
        canalComunicacion.postMessage( data.mensaje );        
    } catch (error) {
        data.dispatchError({ 
            type: data.error, 
 ...
 */
export const recepcionComunicacion = (data: DataCanalRecepcion) => {
    try {
        const canalComunicacion = new BroadcastChannel(data.nombreCanal);

        canalComunicacion.onmessage = (event) => {
            // ¡IMPORTANTE!: Comparamos event.data.type
            if (event.data === data.mensaje && data.dispatchActualizar && data.actualizar) {
